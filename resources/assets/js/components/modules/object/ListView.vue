                ...
                <v-flex xs12>
                    <v-card>
                        <v-card-text>
                            <v-layout row wrap>
                                <v-flex xs12 sm2>
                                    <v-btn @click.native.stop="addObject">
                                        <v-icon ref="identifier">add</v-icon>
                                        <v-icon>mdi-city</v-icon>
                                    </v-btn>
                                    <app-object-add-dialog :position-absolutely="true"
                                                           :show="add"
                                                           @show="val => {add = val}"
                                                           :position-x="x"
                                                           :position-y="y"
                                    ></app-object-add-dialog>
                                </v-flex>
                                <v-flex xs12 sm8>
...
<script lang="ts">
    ...
    @Component({
        'components': {
            'app-object-add-dialog': objectAddDialog
        }
    })
    export default class ListView extends Vue {
        ...
        updateList() {
            this.loading = true;
            return axios.get('/api/v1/objects', {
                params: {
                    'q': this.parameters.q,
                    'v': this.parameters.v,
